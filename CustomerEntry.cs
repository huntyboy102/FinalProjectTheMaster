...
namespace CustomerViewer
{
    public partial class formCustomerEntry : Form
    {
        ...
        /// <summary>
        /// This button saves the listview to a file.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        public void ButtonSaveClick(object sender, EventArgs e)
        {
            ...
            // Display the save dialog if the user clicked OK.
            if (saveLocation.ShowDialog() == DialogResult.OK)
            {
                // Initialize a stream writer.
                FileStream fileToAccess = new FileStream(saveLocation.FileName, FileMode.Create, FileAccess.Write);
                ...
                {
                    // Write each customer's salutation to the text file.
                    writer.Write(customerList[index].GetSalutation() + "\r\n");
                }

                // Go big or go home, CLOSE THE FILE!.
                writer.Close();

                // 
                MessageBox.Show("File has been successfully saved to " + saveLocation.FileName, "Save Successful!", MessageBoxButtons.OK);
            }
            else
            {
